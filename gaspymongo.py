...
def make_atoms_from_doc(doc):
    ...
    if 'array' in doc['atoms']['cell'].keys():
        ...
        cell = doc['atoms']['cell']
    if isinstance(doc['atoms']['atoms'][0]['position'], dict):
        if "pbc" in doc['atoms']['cell'].keys():
            ...
            atoms = Atoms([Atom(atom['symbol'],
                       ...
                       for atom in doc['atoms']['atoms']],
                      cell=cell,
                      pbc=__decode(doc['atoms']['pbc']),
                      info=doc['atoms']['info'],
                      constraint=[dict2constraint(constraint_dict)
                                  for constraint_dict in doc['atoms']['constraints']])
    else:
        if 'pbc' in doc['atoms']['cell']:
            pbc = doc['atoms']['cell']['pbc']
        elif 'pbc' in doc['atoms']:
            pbc = doc['atoms']['pbc']
        atoms = Atoms([Atom(atom['symbol'],
                        atom['position'],
                        tag=atom['tag'],
